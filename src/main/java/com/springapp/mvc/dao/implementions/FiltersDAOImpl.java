...
public class FiltersDAOImpl implements FiltersDAO {
    ...
    public List<HmcBrandFilter> listBrandFilter() {
        return sessionFactory.getCurrentSession().createQuery("from HmcBrandFilter").list();
    }
    ...
    public List<HmcSlidersFilter> listSlidersFilter() {
        return sessionFactory.getCurrentSession().createQuery("from HmcSlidersFilter").list();
    }

    @SuppressWarnings("unchecked")
    public void renewBrandFilter() {
        renew(HmcBrandFilter.class.getSimpleName(), "brand");
    }
    ...
    private void renew(String className, String checkBoxName) {
        ...
        if(className.equals(HmcBrandFilter.class.getSimpleName())) {
            for (String s : set) {
    ...
    private HmcBrandFilter getBrandFilter(String val, int num) {
        HmcBrandFilter pf = new HmcBrandFilter();
        pf.setBrand(val);
        pf.setNum(num);
        return pf;
    }
    ...
    public void renewSlidersFilter() {
        ...
        int[] minMaxProductionYear = roundToNum(getMinMax(listProductionYear), 1);
        ...
        HmcSlidersFilter sf = new HmcSlidersFilter();
        sf.setId(1);
